...
# alias doka='docker stop $(docker ps -a -q) && docker rm $(docker ps -a -q)'
# delete null images
...
alias doclean='docker rmi $(docker images --filter dangling=true -q --no-trunc) && docker rm -v $(docker ps -a -q -f status=exited)'
